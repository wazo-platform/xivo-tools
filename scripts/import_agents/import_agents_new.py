...
def import_csv(agent_cache, agent_ws, skill_ws, xivo):
    ...
    .csv file has to have this header :
        col1,col2,col3,col4 ...
        where col1 are option names from Agent,
        for skills, the col name must be : skill.<category name>.<skill name> 
        (e.g. skill.language.french)
    2. For each row :
        a. Test if this agent exists :
            i.  if the Agent exists, print a warning msg
            ii. if it doesn't, call _create_agent_

    _create_agent_ :
        this create the agent, with the data from the .csv
        if agent has skills, it first create the skill if it doesn't exist
    """
